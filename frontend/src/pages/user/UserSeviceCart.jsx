...
export default function UserServiceCart() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-gray-900 text-gray-100">
      <div className="bg-gray-800 rounded-lg shadow-lg p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-100 mb-4 md:mb-0">Service History</h2>
            ...
            {['pending', 'confirmed', 'completed'].map((s) => (
              <button
                ...
                className={`px-4 py-2 rounded-md font-medium transition-colors ${
                  status === s
                    ? statusButtonColors[s] + ' text-white'
        ...
        {filteredBookings.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-400 text-lg">No {status} services found.</p>
          </div>
        ) : (
                ...
                {filteredBookings.map((booking) => (
                  <tr key={booking.booking_id} className="hover:bg-gray-700 transition-colors">
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-gray-100">{booking.service_name}</div>
                      <div className="text-sm text-gray-400">ID: {booking.booking_id}</div>
                    </td>
                    <td className="px-6 py-4">
                      ...
                      {booking.status === 'completed' && booking.feedback && (
                        <div className="text-sm text-gray-400 mt-1">
                          ...
                          {booking.feedback.substring(0, 50)}...
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <span
                        className={`px-2 inline-flex text-xs font-semibold rounded-full ${statusColors[booking.status]}`}
                      >
      ...
      {mapOpen && mapPos && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl h-96 relative">
            <button
              className="absolute top-4 right-4 bg-gray-700 rounded-full p-2 shadow-md hover:bg-gray-600 text-gray-100"
              onClick={closeMap}
            >
              ×
            </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md relative">
            <button
              ...
              className="absolute top-2 right-3 text-gray-400 hover:text-gray-100 text-2xl"
            >
              ×
            </button>
            ...
            >
              <div className="mb-4">
                ...
                <textarea
                  ...
                  rows={4}
                  required
                  className="w-full border border-gray-700 rounded px-3 py-2 bg-gray-900 text-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500"
              ...
              <button
                ...
                className="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded transition duration-300 disabled:opacity-50"
              >
      ...
      {viewingFeedback && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md relative">
            ...
            <h2 className="text-xl font-semibold text-gray-100 mb-4">Your Feedback</h2>
            <div className="bg-gray-700 p-4 rounded border border-gray-600">
              <p className="text-gray-100">{viewingFeedback}</p>
            </div>
            <div className="mt-4 flex justify-end">
              <button
                onClick={() => setViewingFeedback(null)}
                className="bg-gray-700 hover:bg-gray-600 text-gray-100 px-4 py-2 rounded transition duration-300"
              >
                Close
              </button>
            </div>
