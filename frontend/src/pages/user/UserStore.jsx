  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.data?.items) {
        ...
        // If we get an unexpected response format, use mock data for demonstration
        console.warn('Using mock data due to unexpected response format');
        fetchedProducts = generateMockProducts();
        recommended = fetchedProducts.slice(0, 3);
      }
      ...
      fetchedProducts = fetchedProducts.map(product => ({
        ...
        inventory_amount: product.inventory_amount || 10,
      ...
      setRecommendedProducts(recommended.length ? recommended : fetchedProducts.slice(0, 3));
      ...
      if (retryCount < 3) {
        ...
        // On the third retry, use mock data
        console.warn('Using mock data after failed retries');
        const mockProducts = generateMockProducts();
        setProducts(mockProducts);
        setRecommendedProducts(mockProducts.slice(0, 3));
        
        // Initialize quantities for mock products
        const initialQuantities = {};
  ...
  const handleBuyNow = async (product) => {
    ...
    try {
      ...
      if (product.id.toString().length < 3) {
        ...
        navigate('/user/checkout', { state: { cartItems: [mockCartItem] } });
        return;
      }
      ...
      if (res.status === 200) {
        ...
        navigate('/user/checkout', { state: { cartItems: [itemWithQty] } });
      }
    } catch (error) {
                  ...
                  <NeonCard className="overflow-hidden h-full flex flex-col" color="emerald">
                    ...
                    <div className="relative h-48 overflow-hidden">
                      <motion.img
                        ...
                        onError={(e) => {
                          e.target.src = 'https://images.pexels.com/photos/5503215/pexels-photo-5503215.jpeg?auto=compress&cs=tinysrgb&w=800';
                        }}
                      />
